...
{
    function css_url($path='css_path')
    {
        $CI =& get_instance();
        return base_url() . $CI->config->item($path);
    }
}
...
{
    function less_url($path='less_path')
    {
        $CI =& get_instance();
        return base_url() . $CI->config->item($path);
    }
...
{
    function img_url($path='img_path')
    {
        $CI =& get_instance();
        return base_url() . $CI->config->item($path);
    }
...
{
    function swf_url($path='swf_path')
    {
        $CI =& get_instance();
        return base_url() . $CI->config->item($path);
    }
...
{
    function upload_url($path='upload_path')
    {
        $CI =& get_instance();
        return base_url() . $CI->config->item($path);
    }
...
{
    function download_url($path='download_path')
    {
        $CI =& get_instance();
        return base_url() . $CI->config->item($path);
    }
...
{
    function xml_url($path='xml_path')
    {
        $CI =& get_instance();
        return base_url() . $CI->config->item($path);
    }
 ...
 * @return  string
 */
if ( ! function_exists('manual_url'))
{
    function manual_url($path)
    {
        $CI =& get_instance();
...
{
    function css($file, $media='all', $path, $id=null)
    {
        $id = ($id) ? 'id="'.$id.'"' : '';
        return '<link rel="stylesheet" type="text/css" href="' . css_url($path) . $file . '" media="' . $media . '" ' . $id . '>'."\n";
    }
}
...
{
    function js($file, $path, $atts = array())
    {
...
{
    function img($file, $path, $atts = array())
    {
        $url = '<img src="' . img_url($path) . $file . '"';
        foreach ( $atts as $key => $val )
            $url .= ' ' . $key . '="' . $val . '"';
        $url .= " />\n";
        return $url;
